...
const navLink = [
    ...
    {title:"apply", Link:"/pages/Apply", id:"2"},
    {title:"job", Link:"/pages/Job", id:"3"},
    {title:"result", Link:"/pages/Result", id:"4"}
];
  ...
  return (
    ...
    <div className='sticky top-0 z-30 w-full bg-main backdrop-blur-3xl  print:hidden'>
        <div className='m-auto flex h-16 w-[95%] items-center justify-between md:h-20 lg:w[90%] xl:w[1300px]'>
            <Link href={"/"}>
            <Image src={require("../../../../public/logo.png")} alt='logo' width={90} className='mt-14 w-[70px] sm:mt-20 sm:w-[80px] md:w-[90px]'/>
            </Link>
            <h1 className='text-shadow hidden text-[15px] font-extrabold text-[#b9d8f3] lg:block xl-lg:text-xl xl:text-2xl'>
            Tuition Free Education Program on Latest Technologies
            </h1>
            <h1 className='text-shadow text-[1.125rem] font-extrabold text-[#b9d8f3] lg:hidden'>
            ...
            <div className='hidden items-center gap-5 text-[#faf9f6] sm:flex lg:gap-10'>
                ...
                <div className='relative cursor-pointer' onClick={()=> setShowMenu(!showMenu)}>
                    <div className='flex item-center capitalize '>
                        Courses<IoIosArrowDown className="pl-2 pt-1 size-6" />
                    </div>
                    {showMenu && (
    ...
    <div className={`fixed left-0 top-0 z-40 h-full w-full bg-main print:hidden ${!open && "hidden"}`}>
       ...
       <RxCross1 size={24} className="text-white" onClick={()=>setOpen(false)} />
       </div>             
       <div className="m-auto mt-20 flex w-[90%] flex-col text-base font-normal text-white">
            ...
            <div className={`py-5 ${!showMenu && "border-b"} border-[#146895]`} onClick={()=>setShowMenu(!showMenu)}>
                <div className="flex items-center justify-between cursor-pointer">
                    Courses
                    <IoIosArrowDown size={16} cursor={"pointer"}/>
                </div>
            </div>       
